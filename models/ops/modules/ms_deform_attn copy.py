...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4,fusion=False):
        """
        ...
        self.sampling_offsets = nn.Conv2d(d_model, n_heads * n_levels * n_points * 2, kernel_size=3, padding=1)
        # self.attention_weights = nn.Linear(d_model, n_heads * n_levels * n_points)
        self.attention_weights = nn.Conv2d(d_model, n_heads * n_levels * n_points, kernel_size=3, padding=1)
        # self.value_proj = nn.Linear(d_model, d_model)
        # self.output_proj = nn.Linear(d_model, d_model)
        self.value_proj = nn.Conv2d(d_model, d_model, kernel_size=3, padding=1)
        self.output_proj = nn.Conv2d(d_model, d_model, kernel_size=3, padding=1)
    ...
    def flow_guid_offset(self,flow_forward,flow_backward,offset):
        ...
        return offset
    def flow_guid_offset_fusion(self,flow_forward,flow_backward,offset):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask,flow_forward,flow_backward):
        ...
        value_bs,value_t,value_c,value_h,value_w = input_flatten.shape
        value = self.value_proj(input_flatten.reshape(value_bs*value_t,value_c,value_h,value_w)).reshape(value_bs,value_t,value_c,value_h,value_w)
        ...
        sampling_offsets = sampling_offsets.flatten(3).transpose(2, 3).contiguous().reshape(bs,-1,self.n_heads * self.n_levels * self.n_points * 2)
        
        attention_weights = attention_weights.flatten(3).transpose(2, 3).contiguous().reshape(bs,-1,self.n_heads*self.n_levels * self.n_points)
        ...
        if self.fusion:
            sampling_offsets = self.flow_guid_offset_fusion(flow_forward,flow_backward,sampling_offsets)
        else:
        ...
        output = output.reshape(bs,t,h*w,c).transpose(2, 3).reshape(bs*t,c,h,w)
        
        # output = output.reshape(value_bs,value_t,value_h*value_w,value_c).transpose(2, 3).reshape(value_bs*value_t,value_c,value_h,value_w)
